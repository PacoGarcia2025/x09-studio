...
/** @deprecated use LUCIDE_ICON_SET — mantido para compatibilidade em testes. */
export const KNOWN_LUCIDE_ICONS = LUCIDE_ICON_SET;
...
export function repairKnownRuntimeImportsInSource(content: string): string {
  ...
  const lucideIcons = [
    ...new Set(
      undeclared
        .map((u) => u.name)
        .filter((name) => LUCIDE_ICON_SET.has(name)),
    ),
  ];
